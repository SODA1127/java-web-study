...
/*
 * groupno : 최상위글(원글)과 그글의 답글들을 하나의 구룹으로 묶기위한컬럼
 * step    : 같은그룹내에서의 순서를 가지고있는 컬럼
 * depth   : 답글의 깊이를 가지고있는 컬럼
 */
--새글쓰기(최상위글)
/*
 * groupno = pk : 최상위글(원글)과 그글의 답글들을 하나의 구룹으로 묶기위한컬럼
 * step    =  1 : 같은그룹내에서의 순서를 가지고있는 컬럼
 * depth   =  0 : 답글의 깊이를 가지고있는 컬럼
 */
INSERT INTO
 ...
 * 2. 
 * 	  boardno:시퀀스증가
 *    groupno:원글(답글)의 groupno
 *    step   :원글(답글)의 step+1
 *    depth  :원글(답글)의 depth+1
 */
